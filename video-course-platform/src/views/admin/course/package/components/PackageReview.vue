...
import { getPackageReviews, deletePackageReview, replyPackageReview, type PackageReview } from '@/utils/course-package-storage';
...
const reviews = ref<PackageReview[]>([]);
...
async function loadReviews() {
  ...
  try {
    ...
    reviews.value = getPackageReviews(props.packageId);
  } finally {
    loading.value = false;
  }
}
...
// 删除评价
function handleDeleteReview(review: PackageReview) {
  ElMessageBox.confirm('确认删除该评价吗？', '提示', {
    ...
    .then(() => {
      deletePackageReview(review.reviewId);
...
// 回复评价
function handleReplyReview(review: PackageReview) {
  ElMessageBox.prompt('请输入回复内容', '回复评价', {
    ...
    .then(({ value }) => {
      replyPackageReview(review.reviewId, value);
