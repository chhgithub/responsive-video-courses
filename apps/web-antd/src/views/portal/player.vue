...
const isTrialMode = computed(() => {
  return currentChapter.value && !currentChapter.value.isFree && course.value && (course.value as any).isTrial;
});
...
onMounted(() => {
  ...
  currentChapter.value = chapters.value.find((c) => c.chapterId === chapterId) || chapters.value[0];
});
</script>

<template>
  <div class="video-player-page bg-gray-900 min-h-screen">
    <div class="container mx-auto px-4 py-6">
      <!-- 返回按钮 -->
      <a-button type="text" class="text-white mb-4" @click="backToCourse">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3">
          <div class="bg-black rounded-lg overflow-hidden relative">
            ...
            <!-- 试看提示 -->
            <div v-if="isTrialMode" class="absolute top-4 left-4 bg-orange-500 text-white px-3 py-1 rounded text-sm">
          ...
          <div class="mt-4 text-white">
            ...
            <p class="text-gray-400 mt-2">{{ course?.courseName }}</p>
          </div>
        </div>
        ...
        <div class="lg:col-span-1">
          <div class="bg-gray-800 rounded-lg p-4">
            <h3 class="text-white font-bold mb-4">课程目录</h3>
            <div class="space-y-2 max-h-[600px] overflow-y-auto">
              ...
              >
                <div class="flex items-start gap-2">
                  <span class="text-sm text-gray-400 w-5 shrink-0">{{ chapter.sortOrder }}</span>
                  <div class="flex-1 min-w-0">
                    <div class="font-medium text-sm line-clamp-2">{{ chapter.chapterName }}</div>
