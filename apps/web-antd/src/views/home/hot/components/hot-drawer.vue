...
import type { HotTopicFormState } from '../types';
...
const [BasicDrawer, drawerApi] = useVbenDrawer({
	...
	async onOpenChange(isOpen) {
		...
		if (isUpdate.value && id) {
			const allHotTopics = getHotTopics();
			const data = allHotTopics.find((item) => item.id === id);
			if (data) {
...
async function handleConfirm() {
	try {
		...
		const data = (await formApi.getValues()) as HotTopicFormState;
...
<template>
	<BasicDrawer :title="title" class="w-[600px]">
		...
		<!-- 自定义图片上传区域 -->
		<div class="mb-6 px-6">
			<label class="mb-2 block text-sm font-medium">封面图片</label>
