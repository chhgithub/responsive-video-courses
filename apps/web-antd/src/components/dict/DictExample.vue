...
const difficultyOptions = computed(() =>
  dictStore.getDictOptions(CourseDict.DIFFICULTY),
);
const typeOptions = computed(() => dictStore.getDictOptions(CourseDict.TYPE));
const statusOptions = computed(() =>
  dictStore.getDictOptions(CourseDict.STATUS),
);
const memberLevelOptions = computed(() =>
  dictStore.getDictOptions(MemberDict.LEVEL),
);
const orderStatusOptions = computed(() =>
  dictStore.getDictOptions(OrderDict.STATUS),
);
const paymentMethodOptions = computed(() =>
  dictStore.getDictOptions(OrderDict.PAYMENT_METHOD),
);
...
<template>
  <div class="dict-example p-6">
    <div class="mb-8">
      <h2 class="mb-4 text-2xl font-bold">字典使用示例</h2>
    ...
    <a-card title="课程相关字典" class="mb-6">
      <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
        <!-- 课程难度 -->
        <div>
          <label class="mb-2 block text-sm font-medium">课程难度</label>
          <a-select
        ...
        <!-- 课程类型 -->
        <div>
          <label class="mb-2 block text-sm font-medium">课程类型</label>
          <a-select
        ...
        <!-- 课程状态 -->
        <div>
          <label class="mb-2 block text-sm font-medium">课程状态</label>
          <a-select
    ...
    <a-card title="会员相关字典" class="mb-6">
      <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
        <!-- 会员等级 -->
        <div>
          <label class="mb-2 block text-sm font-medium">会员等级</label>
          <a-radio-group
    ...
    <a-card title="订单相关字典" class="mb-6">
      <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <!-- 订单状态 -->
        <div>
          <label class="mb-2 block text-sm font-medium">订单状态</label>
          <div class="space-x-2">
            <a-tag
              v-for="option in orderStatusOptions"
              :key="option.value"
              :color="option.listClass || 'default'"
            >
              {{ option.label }}
            </a-tag>
          </div>
        ...
        <!-- 支付方式 -->
        <div>
          <label class="mb-2 block text-sm font-medium">支付方式</label>
          <div class="space-x-2">
            <a-tag
              v-for="option in paymentMethodOptions"
              :key="option.value"
              :color="option.cssClass || 'default'"
            >
              {{ option.label }}
            </a-tag>
          </div>
